...
function solve(data::Data)
    ...
    @variable(model, B[V, K] >= 0)
    @variable(model, Q[V, K] >= 0)
    ...
    @constraint(model, [i = V, j = V, k = K; i != j], B[j, k] - ((B[i, k] + s[i] + t[i, j]) - M1*(1 - x[i, j, k])) >= 0)
    @constraint(model, [i = V, k = K], B[i, k] >= e[i])
    @constraint(model, [i = V, k = K], B[i, k] <= l[i])
    @constraint(model, [i = V, j = V, k = K; i != j], Q[j, k] >= (Q[i, k] + q[i]) - M2*(1 - x[i, j, k]))
    @constraint(model, [i = V, k = K], Q[i, k] <= C)
    @constraint(model, [i = P, k = K], B[i + n, k] >= B[i, k])
    @constraint(model, [i = P, k = K], B[i + n, k] - (B[i, k] + s[i]) <= L)
